...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func makeBall() {
        ...
        // ignores all forces and impulse
        ball.physicsBody?.isDynamic = false
        // use precise collison collection detection
        ball.physicsBody?.usesPreciseCollisionDetection = true
        // no loss energy from friction
        ball.physicsBody?.friction = 0
        // gravaty is not a factor
        ball.physicsBody?.affectedByGravity = false
        // bounce fully off of other object
        ball.physicsBody?.restitution = 1
        // does not slow down down over time
        ball.physicsBody?.linearDamping = 0
        ball.physicsBody?.contactTestBitMask = (ball.physicsBody?.collisionBitMask)!
        
        addChild(ball) // add ball object
    }
    ...
    func makeBricks() {
        ...
        bricks.removeAll() // Clear array
        removedBricks = 0 // Reset counter
